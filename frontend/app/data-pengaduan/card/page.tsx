  ...
  useEffect(() => {
    const fetchStatusCounts = async () => {
      try {
        ...
        const onprogressResponse = await fetch("http://localhost:5000/api/pengaduan/count/progress");

        const pendingData = await pendingResponse.json();
        const approvedData = await approvedResponse.json();
        const rejectedData = await rejectedResponse.json();
        const completedData = await completedResponse.json();
        const progressData = await onprogressResponse.json();

        // Set data masing-masing sesuai dengan kunci yang tepat
        setStatusCounts({
          pending: pendingData.count,
          approved: approvedData.count, // Update kunci sesuai respons API
          rejected: rejectedData.count, // Update kunci sesuai respons API
          completed: completedData.count, // Update kunci sesuai respons API
          onprogress: progressData.count, // Update kunci sesuai respons API
        });
      } catch (error) {
